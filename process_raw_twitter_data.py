...
def process_data(args):
    ...
    recognized_tweets = 0
    result_dict = {}
    for i, file in enumerate(sorted(os.listdir(directory_path))):
...
def add_to_result(result_dict, date, location):
    if date not in result_dict:
        result_dict[date] = {country.name.lower(): 0 for country in pycountry.countries}
...
def save_data(result_dict, location):
    for day in result_dict.keys():
        ...
        df = pd.DataFrame(data.items(), columns=['country', 'tweets'])
